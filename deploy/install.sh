...
setup_admin() {
    ...
    # Append [admin] section to config.toml
    cat >> "$config_file" << ADMIN
...
ADMIN
    ...
    echo -e "  \e[1;32m╠══════════════════════════════════════════════════╣\e[0m"
    echo -e "  \e[1;32m║\e[0m  URL:      http://localhost:9090/admin/          \e[1;32m║\e[0m"
...
migrate_legacy_token() {
    ...
    if grep -qE '^\s*auth_token\s*=' "$config_file" && ! grep -q '^\[tokens\.' "$config_file"; then
        ...
        if [[ -n "$auth_token" && "$auth_token" != '${'* ]]; then
            ...
            local token_hash
            token_hash=$("$INSTALL_DIR/venv/bin/python" -c "
import hashlib
token = '''$auth_token'''
...
do_set_admin_password() {
    ...
    local password
    if [[ -t 0 ]]; then
        read -rsp "Enter new admin password (min 8 chars): " password
        echo
    ...
    # Update or create [admin.users.admin] in config
    if grep -q '^\[admin\.users\.admin\]' "$config_file"; then
        # Replace existing password_hash line
        sed -i "/^\[admin\.users\.admin\]/,/^\[/ s|^password_hash = .*|password_hash = \"$password_hash\"|" "$config_file"
    else
        # Append admin user section
        if ! grep -q '^\[admin\]' "$config_file"; then
            echo -e "\n[admin]\nenabled = true\nport = 9090\nhost = \"127.0.0.1\"\n" >> "$config_file"
        fi
        echo -e "\n[admin.users.admin]\npassword_hash = \"$password_hash\"\nrole = \"admin\"" >> "$config_file"
    fi
